...
/**
 * @brief Pack a command_ack message (inline)
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 *
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_command_ack_pack_inline(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint16_t command, uint8_t result)
{
	mavlink_command_ack_t *packet = (mavlink_command_ack_t *) _MAV_PAYLOAD_NON_CONST(msg);
	packet->command = command;
	packet->result = result;
