...
/**
 * @brief Pack a param_request_list message (inline)
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 *
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_param_request_list_pack_inline(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t target_system, uint8_t target_component)
{
	mavlink_param_request_list_t *packet = (mavlink_param_request_list_t *) _MAV_PAYLOAD_NON_CONST(msg);
	packet->target_system = target_system;
	packet->target_component = target_component;
