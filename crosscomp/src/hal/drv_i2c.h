...
void i2cReadBuffer_start(uint8_t addr_, uint8_t reg_, uint8_t len, uint8_t *buf) {
  timeout = I2C_DEFAULT_TIMEOUT;
  addr = addr_;
  reg = reg_;
  writing = 0;
...
bool i2cReadBuffer_busy() {
  bool res = (busy && --timeout > 0);
  if (!res) {
    if (timeout == 0) {
      i2cErrorCount++;
      error = true;
...
bool i2cReadBuffer_wait() {
  while (i2cReadBuffer_busy());
  return !error;
}
