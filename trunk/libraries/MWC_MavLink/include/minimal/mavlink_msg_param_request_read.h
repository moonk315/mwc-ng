...
/**
 * @brief Pack a param_request_read message (inline)
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 *
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_param_request_read_pack_inline(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t target_system, uint8_t target_component, const char *param_id, int16_t param_index)
{
	mavlink_param_request_read_t *packet = (mavlink_param_request_read_t *) _MAV_PAYLOAD_NON_CONST(msg);
	packet->param_index = param_index;
	packet->target_system = target_system;
	packet->target_component = target_component;
        mav_array_memcpy(packet->param_id, param_id, sizeof(char)*16);
