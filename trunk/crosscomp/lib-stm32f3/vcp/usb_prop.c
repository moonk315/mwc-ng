...
/* текущий указатель выгрузки символа getchar */
volatile uint8_t* pGetChar_ptr_out;
/* количество байт в буфере getchar */
...
void Virtual_Com_Port_init(void)
{
  ...
  /* начальная установка буфера приёма/передачи  */
  VCP_Rx_ptr_in = 0;
  //pGetChar_ptr_out = USB_Rx_Buffer;	  				// установка текущего указателя на начало буфера
  //pGetChar_ptr_max = USB_Rx_Buffer;					 // буфер пуст
...
void Handle_USBAsynchXfer (void)
{
  ...
  if(USB_Tx_State != 1)
  {
    ...
    /* если длинна пакета больше длины буфера USB */
